...
Tests that call `file::open` or `file::open_dir` belong in `compile_only`
because those APIs use the native-only `syscall` intrinsic. Standard-stream
coverage remains in `runtime` so both interpreters continue to exercise
`file_std*`.
