...
GC write barriers are inserted by `src/write_barriers.rs` as a textual rewrite of the LLVM IR (so barrier calls don't block allocation elision/SROA). In release the pass runs *after* `opt -O3` and the final `clang -O3` link inlines the fast path; in debug the pass runs on `-emit-llvm` IR before the (LTO-deferred) ASAN instrumentation, so debug/ASAN test binaries also exercise the collector with barriers active. The pass instruments `store ptr` (and `<N x ptr>` vector stores) into non-stack destinations, and `llvm.memcpy`/`memmove` (via a bulk `sol_gc_memcpy_barrier`); it appends `declare`s for those runtime functions when the module doesn't already define them. `sol_memcpy` shades its copied region itself.

The collector (`solar-system/src/gc.rs`) is **concurrent**: a dedicated GC thread spawned in `sol_start` owns every cycle. Mutators never collect — on the heap-growth heuristic `sol_alloc` calls `request_gc` (a futex wake) and keeps running. A cycle is: **STW pause 1** (signal all registered mutators, scan their stacks+registers, copy root pointer values into the gray queue, set `SOL_CONCURRENT_MARKING`, resume) → **concurrent mark** (the GC thread drains the gray queue while mutators run; the Dijkstra insertion barrier and `sol_memcpy` shade newly-stored pointers via per-thread gray buffers; new allocations are *born black*) → **STW pause 2** (clear the flag, flush gray buffers + re-scan roots, drain to fixpoint, then the parallel sweep). The gray frontier is a queue (`GRAY` + per-thread buffers in `ThreadSlot`) kept separate from the alloc/mark bitmaps. Known limitations: no allocation back-pressure, single-threaded concurrent marking, and marker heap reads are racy-by-design (sound on x86 for aligned words).
