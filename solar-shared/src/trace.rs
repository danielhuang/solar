...
pub fn demangle_solar(symbol: &str) -> Option<String> {
...
/// Demangle a concrete Solar type symbol, including generic arguments.
pub fn pretty_name(inner: &str) -> String {
...
/// Capture instruction addresses without looking up symbols or debug information.
pub fn capture() -> Vec<usize> {
    let mut addresses = Vec::with_capacity(32);
    backtrace::trace(|frame| {
        // Saved instruction pointers address the instruction after the call.
        addresses.push((frame.ip() as usize).saturating_sub(1));
        true
    });
    addresses
}

/// Resolve one instruction address, including demangled names, source locations,
/// and any inlined frames. Unknown addresses retain their hexadecimal value.
pub fn resolve_address(address: usize) -> String {
    use std::fmt::Write;
    ...
    backtrace::resolve(address as *mut std::ffi::c_void, |symbol| {
        ...
        let name = symbol
            ...
            .map(|name| {
                name.as_str()
                    .and_then(demangle_solar)
                    .unwrap_or_else(|| name.to_string())
            })
            .unwrap_or_else(|| "<unknown>".into());
        let _ = write!(text, "{address:#x} {name}");
        if let Some(file) = symbol.filename() {
            let _ = write!(text, " at {}", file.display());
    ...
    if text.is_empty() {
        let _ = write!(text, "{address:#x} <unknown>");
    }
    text
}

/// Resolve saved addresses only when a trace is displayed, including inline frames.
pub fn format(addresses: &[usize]) -> String {
    use std::fmt::Write;
    let mut text = String::from("Stack trace:\n");
    for (index, &address) in addresses.iter().enumerate() {
        for line in resolve_address(address).lines() {
            let _ = writeln!(text, "  {index}: {line}");
        }
    }
    text
}
